...
def rimg_inference(rimg_file_list, output_directory, check_point_pth) : #inference 
    ...
    for rimg_file in tqdm(rimg_file_list, file = sys.stdout) :
            ...
            #check_point2 = torch.load('./models/trained/iln_0d/iln_65_best.pth')
            # Model
            #breakpoint()
            model = generate_model(check_point['model']['name'], check_point['model']['args'])
            #model2 = generate_model(check_point2['model']['name'], check_point2['model']['args'])
            ...
            lidar_in = {'max_r': 100.0, 'min_r': 5.0, 'norm_r': 100.0, 
                        'max_v': 20.3*np.pi/180, 'min_v': -21.05*np.pi/180, 
                        'max_h': 3.141592653589793, 'min_h': -3.141592653589793, 
                        'frequency': 10, 'max_search': 100.0, 'channels': 32, 
                        'points_per_ring': 2048}
            lidar_out = copy.deepcopy(check_point['lidar_in'])
            lidar_out['channels'] = 128
            lidar_out['points_per_ring'] = 2048
...
if __name__ == '__main__':
    ...
    rimg_file_list_origin = sorted(glob.glob('./rimg_dataset/sample/Lidar_sr_data/32_2048_origin/train/*.rimg'))
    rimg_file_list_downsample = sorted(glob.glob('./rimg_dataset/sample/Lidar_sr_data/32_2048_downsample/train/*.rimg'))

    output_directory_origin = './inference/rimg/origin_32_to_128_sample_hyok/'
    output_directory_downsample = './inference/rimg/downsample_32_to_128_sample_hyok/'

    output_pcd_dir_origin = './inference/pcd/origin_32_to_128_sample_hyok/'
    output_pcd_dir_downsample = './inference/pcd/downsample_32_to_128_sample_hyok/'

    denoise_rimg_origin = './inference/rimg/denoise/origin_32_to_128_sample_hyok/'
    denoise_rimg_downsample = './inference/rimg/denoise/downsample_32_to_128_sample_hyok/'

    #check_point_pth = glob.glob('./models/trained/iln_0d_custom1/*best.pth')  
    check_point_pth = glob.glob('./models/pt_weights/iln/*best.pth')  
    # check_point_pth = glob.glob('./models/trained/iln_0d_custom1/pretrain_weight.pth')
