...
class RangeImagesDataset(Dataset):
    def __init__(self, train, directory, scene_ids, res_in, res_out, memory_fetch=True):
        """
        ...
        self.res_in = res_in #32_1024
        self.res_out = res_out #128_1024
        ...
        for scene_id in scene_ids:
            ##train
            if self.train :
                input_directory = os.path.join(directory, scene_id, res_in) #./rimg_dataset/Custom/Airl_1/32_1024
            ...
            ##valid
                ...
                input_directory = os.path.join(directory, scene_id, res_in) #./rimg_dataset/Custom/Airl_1/32_1024_val
                input_filenames = [os.path.join(input_directory, f) for f in os.listdir(input_directory) if f.endswith('.rimg')]
                input_filenames.sort()
    ...
    def __getitem__(self, item):
        ...
        #input_range_image=input_range_image.reshape(32, 1024)
        #output_range_image=output_range_image.reshape(128, 1024)
        #print("\nTrain data = {}\nLabel data ={}".format(input,output), file=sys.stdout, flush=True)
        input.close()
        output.close()
