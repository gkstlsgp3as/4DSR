...
train_output_path_origin_32 = './rimg_dataset/Custom_2/Lidar_sr_data/32_1024_origin/train/'
...
valid_output_path_origin_32 = './rimg_dataset/Custom_2/Lidar_sr_data/32_1024_origin/valid/'
...
def make_range_image(file_list, file_resolution, data_type):
    ...
    for i in tqdm(range(0, len(file_list))) :
        ...
        # if range_image.shape[1] == 128 :
        #     low_res_index = range(0, 128, 4) 
        #     range_image_32 = range_image[:,low_res_index]
        #     # range_image_32 = range_image_32.transpose()
        #     # range_image = np.flip(range_image, axis=0)
        #     # print(range_image_32.shape)

        #     if not os.path.exists(output_path_downsaple_32):
        #         os.makedirs(output_path_downsaple_32)

        #     fw = open(output_path_downsaple_32 + str(i).zfill(5) + '.rimg', 'wb')
        #     size = struct.pack('QQ', range_image_32.shape[1], range_image_32.shape[0])
        #     fw.write(size)
        #     for width in range(0,range_image_32.shape[0]):      #1024
        #         for ch in range(0,range_image_32.shape[1]):  #32
        #             L = range_image_32[width,ch]
        #             s = np.float16(L).tobytes()
        #             fw.write(bytes([s[0],s[1]]))
        #     fw.close()
...
def read_pcd():
    ...
    for i in train_dir_num : 
        file_list_train_128 = file_list_train_128 + sorted(glob.glob('../../super_resolution_data/49-1_Super_Resolution/High_Resolution/img/sequences/'+i+'/velodyne/*.pcd'))
        file_list_train_32 = file_list_train_32 + sorted(glob.glob('../../super_resolution_data/49-1_Super_Resolution/Low_Resolution/img/sequences/'+i+'/velodyne/*.pcd'))

    for i in valid_dir_num : 
        file_list_valid_128 = file_list_valid_128 + sorted(glob.glob('../../super_resolution_data/49-1_Super_Resolution/High_Resolution/img/sequences/'+i+'/velodyne/*.pcd'))
        file_list_valid_32 = file_list_valid_32 + sorted(glob.glob('../../super_resolution_data/49-1_Super_Resolution/Low_Resolution/img/sequences/'+i+'/velodyne/*.pcd'))
