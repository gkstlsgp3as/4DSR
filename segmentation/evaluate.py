...
def main() -> None:
    ...
    parser.add_argument('--pre', type=str, help='pretrained model path')
    ...
    if args.pre:
        loaded_state_dict = torch.load(args.pre,
    ...
    callbacks = Callbacks([
        SaverRestore(),
        MeanIoU(configs.data.num_classes, configs.data.ignore_label,True)
    ])
...
if __name__ == '__main__':
    ...
    main()
    now = datetime.now()
    print(now.isoformat().split('T')[0],now.isoformat().split('T')[1])
